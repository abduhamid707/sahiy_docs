...
import { ArrowLeft, Save, Trash2, Eye, ImagePlus } from "lucide-react";
import Link from "next/link";
import { ImageUpload } from "@/components/shared/ImageUpload";

import { ALL_ROLES } from "@/lib/constants";

export default function EditDocForm({ doc, projects, categories }: { doc: any, projects: any[], categories: any[] }) {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [selectedProjectId, setSelectedProjectId] = useState(String(doc.projectId || ""));
  ...
  const [keywordsInput, setKeywordsInput] = useState(doc.keywords?.join(", ") || "");
  const [isAddingCategory, setIsAddingCategory] = useState(false);
  const [newCategoryName, setNewCategoryName] = useState("");

  // Filter categories based on selected project
  const filteredCategories = categories.filter(cat => String(cat.projectId) === selectedProjectId);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/admin/docs/${doc._id}`, {
        method: "PATCH",
      ...
      if (!res.ok) throw new Error("Hujjatni yangilashda xatolik yuz berdi");

      toast.success("Hujjat muvaffaqiyatli yangilandi");
      router.push("/admin/docs");
      router.refresh();
    } catch (error) {
      toast.error("Hujjatni yangilashda xatolik yuz berdi");
      console.error(error);
    } finally {
      setLoading(false);
    }
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-1000">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h1 className="text-3xl font-black text-foreground tracking-tight">Hujjatni tahrirlash</h1>
            <p className="text-muted-foreground font-medium">Mavjud texnik hujjat mazmuni va ruxsatlarini o'zgartirish.</p>
          </div>
        </div>
        <Link 
          href={`/docs/${doc._id}`}
          className={cn(
            ...
            "h-12 px-6 rounded-2xl border-border font-bold text-sm bg-card hover:bg-muted transition-all active:scale-95 gap-2 hidden md:flex"
          )}
          target="_blank"
        >
        ...
        <div className="lg:col-span-2 space-y-8">
          <Card className="rounded-[2.5rem] border-border shadow-xl shadow-slate-900/5 dark:shadow-none overflow-hidden bg-card">
            <CardHeader className="bg-muted/50 border-b border-border px-8 py-6">
              <CardTitle className="text-sm font-black uppercase tracking-widest text-muted-foreground">Hujjat tarkibi</CardTitle>
            </CardHeader>
          ...
          <Card className="rounded-[2.5rem] border-border shadow-xl shadow-slate-900/5 dark:shadow-none overflow-hidden lg:sticky lg:top-24 bg-card">
            ...
            <CardContent className="space-y-8 px-8 py-8">
                ...
                >
                  <SelectTrigger className="h-11 rounded-xl border-border font-bold text-xs bg-card shadow-sm">
                    <SelectValue placeholder="Loyihani tanlang">
                      {selectedProjectId ? projects.find(p => String(p._id) === selectedProjectId)?.name : "Loyihani tanlang"}
                    </SelectValue>
                  </SelectTrigger>
                  <SelectContent className="rounded-2xl border-border shadow-2xl bg-card">
                    {projects.map((proj) => (
                      <SelectItem key={String(proj._id)} value={String(proj._id)} className="rounded-xl text-xs font-bold py-2 focus:bg-blue-500/10 focus:text-blue-600 dark:focus:text-blue-400">
                        {proj.name}
                      </SelectItem>
                    ))}
                  </SelectContent>
                  ...
                  >
                    <SelectTrigger className="h-11 rounded-xl border-border font-bold text-xs bg-card shadow-sm disabled:bg-muted disabled:opacity-50">
                      <SelectValue placeholder={selectedProjectId ? "Kategoriyani tanlang" : "Loyihani tanlang"}>
                        {formData.categoryId ? categories.find(c => String(c._id) === formData.categoryId)?.name : (selectedProjectId ? "Kategoriyani tanlang" : "Loyihani tanlang")}
                      </SelectValue>
                    </SelectTrigger>
                    <SelectContent className="rounded-2xl border-border shadow-2xl bg-card">
                      {filteredCategories.map((cat) => (
                        <SelectItem key={String(cat._id)} value={String(cat._id)} className="rounded-xl text-xs font-bold py-2 focus:bg-blue-500/10 focus:text-blue-600 dark:focus:text-blue-400">
                          {cat.name}
                        </SelectItem>
                      ))}
                      {selectedProjectId && (
              ...
              <div className="pt-6 border-t border-border flex flex-col gap-4">
                <Button 
                  type="submit" 
                  className="w-full h-14 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white font-bold uppercase tracking-wider shadow-xl shadow-blue-500/20 active:scale-95 transition-all disabled:bg-muted disabled:text-muted-foreground" 
                  disabled={loading}
                >
                ...
                <Button 
                  type="button" 
                  variant="ghost" 
                  className="w-full h-12 rounded-xl text-rose-600 hover:text-rose-700 hover:bg-rose-500/10 font-bold transition-all"
                >
