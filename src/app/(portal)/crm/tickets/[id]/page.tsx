...
export default async function CrmTicketPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [messages, previousTickets, agents] = await Promise.all([
  ...
  return <CrmTicketDetail ticket={JSON.parse(JSON.stringify(ticket))} messages={JSON.parse(JSON.stringify(messages))} previousTickets={JSON.parse(JSON.stringify(previousTickets))} agents={JSON.parse(JSON.stringify(agents))} currentUser={user} canManage={canSeeAllTickets(user)} nowIso={new Date().toISOString()} />;
}
