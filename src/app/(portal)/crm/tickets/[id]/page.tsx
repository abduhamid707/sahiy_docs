...
export default async function CrmTicketPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!ticket) notFound(); if (!canAccessTicket(user, ticket)) redirect("/crm");
  const [messages, previousTickets, agents, tasks] = await Promise.all([
    ...
    TicketTask.find({ ticketId: id }).populate("assignedTo", "name email image").populate("createdBy", "name").sort({ deadlineAt: 1 }).lean(),
  ]);
