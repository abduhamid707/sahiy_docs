...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (message.sender.toString() !== session.user.id) return NextResponse.json({ success: false, error: "Faqat o‘z xabaringizni tahrirlashingiz mumkin" }, { status: 403 });
...
export async function DELETE(_: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const ownMessage = message.sender.toString() === session.user.id;
