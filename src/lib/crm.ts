...
export const CRM_STATUS_LABELS: Record<string, string> = { OPEN: "Yangi", NEW: "Yangi", IN_PROGRESS: "Jarayonda", WAITING: "Kutilmoqda", RESOLVED: "Hal qilindi", CLOSED: "Yopildi" };
export const CRM_PRIORITY_LABELS: Record<string, string> = { LOW: "Past", NORMAL: "Oddiy", HIGH: "Yuqori", CRITICAL: "Kritik" };
export const CRM_CATEGORY_LABELS: Record<string, string> = {
  DELIVERY_DELAY: "Buyurtma kechikishi", TRACKING: "Tracking muammosi", NOT_RECEIVED: "Mahsulot kelmagan",
  WRONG_OR_MISSING: "Noto'g'ri / yetishmaydi", REFUND_PAYMENT: "Refund / to'lov", CHINA_WAREHOUSE: "Xitoy ombori", OTHER: "Boshqa",
};

export function isOpenStatus(status: string) { return !["RESOLVED", "CLOSED"].includes(status); }
export function isOverdue(ticket: { status: string; deadlineAt?: string | Date | null }, now: string | Date | number = Date.now()) { return isOpenStatus(ticket.status) && !!ticket.deadlineAt && new Date(ticket.deadlineAt).getTime() < new Date(now).getTime(); }
export function ticketPublicId(ticket: { ticketNumber?: string; _id?: unknown }) { return ticket.ticketNumber || `TKT-${String(ticket._id || "").slice(-6).toUpperCase()}`; }
