...
import { taskTimeLabel, taskUrgency, taskUrgencyScore } from "@/lib/crmTasks";
import { cn } from "@/lib/utils";

const taskFilters = [
  ["MY", "Menga biriktirilgan"], ["OVERDUE", "Overdue"], ["HOUR", "Keyingi 1 soat"],
...
export default function CrmTaskOverview({ initialTasks, initialNotifications, currentUserId, nowIso }: any) {
  const [filter, setFilter] = useState("MY");
  const [notifications, setNotifications] = useState(initialNotifications);
  const [pushLoading, setPushLoading] = useState(false);
  const unread = notifications.filter((item: any) => !item.readAt).length;
  const tasks = useMemo(() => [...initialTasks].filter((task: any) => {
    const urgency = taskUrgency(task.deadlineAt, task.status, new Date(nowIso));
    if (filter === "MY") return task.assignedTo?._id === currentUserId && !["DONE", "CANCELLED"].includes(task.status);
    ...
    return (task.sentEvents || []).length > 0 && urgency !== "DONE";
